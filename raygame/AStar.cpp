...
std::vector<Node*> AStar::aStar(Node * startNode, Node * endNode)
{
	//Validate the input
	...
	while (!openList.empty()) {
		...
		for (Edge e : currentNode->connections) {
			...
			//Otherwise the target node is in the openList
			else 
			{
				...
				if (currentNode->fScore < e.target->fScore)
				{
					...
					e.target->gScore = currentNode->gScore + e.cost;
					// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
					e.target->fScore = currentNode->fScore + e.cost;
					//Set the target node's previous to currentNode
					e.target->previous = currentNode;
				}
			}
...
void AStar::drawNode(Node * node, bool selected)
{
	...
	if (selected)
	{
		DrawCircle(node->position.x, node->position.y, 20, BROWN);
	}
	else
	{
		DrawCircle(node->position.x, node->position.y, 20, BLACK);
	}
	//Draw the text
	DrawText(buffer, node->position.x - 5, node->position.y - 5, 15, WHITE);
}

void AStar::drawGraph(Node * node, std::vector<Node*>* drawnList)
{
	drawNode(node);
	...
	for (Edge e : node->connections) {
		...
		DrawText(buffer, costPos.x, costPos.y, 20, WHITE);
